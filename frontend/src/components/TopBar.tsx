...
const TopBar: React.FC<TopBarProps> = ({ onLogout }) => {
  return (
    ...
    >
      ...
      {/* Right — User */}
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2.5 bg-white/[0.04] px-3 py-1.5 rounded-xl border border-white/[0.08]">
          ...
          <span className="text-sm font-medium text-white/50 hidden sm:block">Aditya</span>
        </div>
        <button
          onClick={onLogout}
          className="w-8 h-8 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-white/50 hover:text-ll-red hover:border-ll-red/20 hover:bg-ll-red/[0.05] transition-all duration-200"
        >
          <Power size={14} />
        </button>
      </div>
